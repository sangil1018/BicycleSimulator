...
public class TrafficLightEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (Application.isPlaying)
        {
            ...
            EditorGUILayout.Space(2);
            EditorGUILayout.LabelField("보행 신호 강제 (Play mode only):", EditorStyles.miniBoldLabel);
            using (new EditorGUILayout.HorizontalScope())
            {
                var prev = GUI.backgroundColor;

                GUI.backgroundColor = new Color(1f, 0.3f, 0.3f);
                if (GUILayout.Button("■ 빨강"))
                    light.ForcePedestrianState(PedestrianState.Red);

                GUI.backgroundColor = new Color(0.3f, 1f, 0.4f);
                if (GUILayout.Button("■ 초록"))
                    light.ForcePedestrianState(PedestrianState.Green);
        ...
        {
            EditorGUILayout.HelpBox("강제 전환 버튼은 Play mode에서 사용 가능합니다.", MessageType.None);
        }
    }
