...
namespace AssetTools.Editor
{
    ...
    public static class ShaderAudit
    {
        /// <summary>
        /// 감사 결과 전문을 만든다.
        /// 진입점은 Tools > Asset Tools 창의 「셰이더 감사」 탭 하나뿐이다.
        /// (예전에는 Tools/Shaders 메뉴에도 같은 기능이 있었으나 중복이라 정리했다)
        /// </summary>
        public static string BuildReport()
        {
            ...
            sb.AppendLine("  변형 로깅을 켜서 무엇이 언제 컴파일되는지부터 확인할 것.");
        ...
        // 켜고 끄는 조작은 Asset Tools 창의 「셰이더 감사」 탭 체크박스에서 한다.

        /// <summary>현재 컴파일 로깅 상태. 그래픽스 설정을 직접 읽어 실제 값을 돌려준다.</summary>
        public static bool IsCompileLogEnabled()
        {
            var prop = FindCompileLogProperty(out _);
            return prop != null && prop.boolValue;
        }

        public static void SetCompileLog(bool on)
        {
            var prop = FindCompileLogProperty(out var so);
            if (prop == null)
            {
        ...
        static SerializedProperty FindCompileLogProperty(out SerializedObject so)
        {
            so = new SerializedObject(
                AssetDatabase.LoadAllAssetsAtPath("ProjectSettings/GraphicsSettings.asset")[0]);
            return so.FindProperty("m_LogWhenShaderIsCompiled");
        }
    }
}
