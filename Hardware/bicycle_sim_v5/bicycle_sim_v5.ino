// ================================================================
//  자전거 안전체험 시뮬레이터  ESP32-S3 펌웨어 v5.5 (통합 완성본)
//  빛고을국민안전체험관 / FLUXION / 2026.06
//  v5.5: DMP 조향 + v4.1 모든 부가 기능(RGB, 진동, 명령) 통합
// ================================================================
...
void processDMP()
{
    ...
    if ((icm.status == ICM_20948_Stat_Ok) || (icm.status == ICM_20948_Stat_FIFOMoreDataAvail))
    {
        if ((data.header & DMP_header_bitmap_Quat6) > 0)
        {
            ...
            if (fabsf(yaw - g_lastRawYaw) > 0.08f)
            {
                g_lastRawYaw = yaw;
...
void setup()
{
    ...
    g_bootMs = millis();
    Serial.println("{\"debug\":\"DMP v5.5 Ready. Stabilizing for 5s...\"}");
